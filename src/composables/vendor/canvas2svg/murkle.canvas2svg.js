...
/* eslint-disable */
(function () {
  let STYLES; let ctx; let CanvasGradient; let CanvasPattern; let
    namedEntities;

  // helper function to format a string
  function format(str, args) {
    const keys = Object.keys(args); let
      i;
  ...
  function randomString(holder) {
    let chars; let randomstring; let
      i;
    if (!holder) {
  ...
  function createNamedToNumberedLookup(items, radix) {
    let i; let entity; const lookup = {}; let base10; let
      base16;
    items = items.split(',');
    radix = radix || 10;
  ...
  function getDominantBaseline(textBaseline) {
    ...
    const mapping = {
      alphabetic: 'alphabetic', hanging: 'hanging', top: 'text-before-edge', bottom: 'text-after-edge', middle: 'central',
    };
  ...
  CanvasGradient.prototype.addColorStop = function (offset, color) {
    const stop = this.__ctx.__createElement('stop'); let regex; let
      matches;
   ...
   */
  ctx = function (o) {
    const defaultOptions = { width: 500, height: 500, enableMirroring: false }; let
      options;
  ...
  ctx.prototype.__createElement = function (elementName, properties, resetFill) {
    ...
    const keys = Object.keys(properties); let i; let
      key;
    if (resetFill) {
  ...
  ctx.prototype.__setDefaultStyles = function () {
    ...
    const keys = Object.keys(STYLES); let i; let
      key;
    for (i = 0; i < keys.length; i++) {
      key = keys[i];
  ...
  ctx.prototype.__applyStyleState = function (styleState) {
    const keys = Object.keys(styleState); let i; let
      key;
    for (i = 0; i < keys.length; i++) {
      key = keys[i];
  ...
  ctx.prototype.__getStyleState = function () {
    let i; const styleState = {}; const keys = Object.keys(STYLES); let
      key;
    for (i = 0; i < keys.length; i++) {
      key = keys[i];
  ...
  ctx.prototype.__applyStyleToCurrentElement = function (type) {
    ...
    const keys = Object.keys(STYLES); var i; let style; let value; let id; let regex; let
      matches;
    for (i = 0; i < keys.length; i++) {
      style = STYLES[keys[i]];
      value = this[keys[i]];
      if (style.apply) {
        ...
        if (value instanceof CanvasPattern) {
          ...
          if (value.__ctx) {
            // copy over defs
            const len = value.__ctx.__defs.childNodes.length;
            for (var i = 0; i < len; i++) {
  ...
  ctx.prototype.getSerializedSvg = function (fixNamedEntities) {
    ...
    let keys; let i; let key; let value; let regexp; let
      xmlns;
  ...
  ctx.prototype.__parseFont = function () {
    const regex = /^\s*(?=(?:(?:[-a-z]+\s*){0,2}(italic|oblique))?)(?=(?:(?:[-a-z]+\s*){0,2}(small-caps))?)(?=(?:(?:[-a-z]+\s*){0,2}(bold(?:er)?|lighter|[1-9]00))?)(?:(?:normal|\1|\2|\3)\s*){0,3}((?:xx?-)?(?:small|large)|medium|smaller|larger|[.\d]+(?:\%|in|[cem]m|ex|p[ctx]))(?:\s*\/\s*(normal|[.\d]+(?:\%|in|[cem]m|ex|p[ctx])))?\s*([-_,\'\"\sa-z0-9]+?)\s*$/i;
  ...
  ctx.prototype.drawImage = function () {
    ...
    let currentElement; let svgImage; let canvas; let context; let
      id;
